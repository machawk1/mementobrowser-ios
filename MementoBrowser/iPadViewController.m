...
- (void)viewDidLoad
{
    ...
    //Initialize arrays for going back
    ...
    // Initialize the subscript that selects which memento to display
...
- (IBAction)getDate:(id)sender {
    ...
    if (self.datePicker.superview == nil || [openPicker.title isEqualToString:@"Date"]) {
        ...
        datePicker = [[UIDatePicker alloc] initWithFrame:CGRectMake(0, 0, 20, 20)];
        ...
        CGRect startRect = CGRectMake(0, 0, pickerSize.width, pickerSize.height);
        ...
        [popover setPopoverContentSize:CGSizeMake(pickerSize.width, pickerSize.height) animated:NO];
        [popover presentPopoverFromBarButtonItem:self.openPicker permittedArrowDirections:UIPopoverArrowDirectionAny animated:YES];
        //DO NOT CHANGE THIS LINE.  xcode thinks there is a problem but there isn't.
...
{
    //Enables proper buttons and gives them the correct names
    openPicker.title = @"Date";
    nowButton.title = @"Now";
    actualDate = oldDate;
    btnActualDate.text = [dateFormatter stringFromDate:actualDate];
